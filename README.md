...
現在私が行っている業務で、紙面のアンケート結果などをExcelへ転記する、といった作業があります。これを手作業で行うのは時間的にも精神的にも負担がかかるのでどうにかしてツールを使って自動化できないか、と考えている内に光学文字認識（OCR）に興味を持ちました。  
...
傾き補正は VisionAPI によって文字を検出し、文字を囲む最小外接矩形の傾きをx軸に対して垂直になるように補正することで行います。
その後もう一度 VisionAPI を呼び出し、前工程で処理した画像データに対して文字認識を行います。認識した文字を配列に格納することで後工程の正規表現を用いたデータ抽出を行えるようにしています。

傾き補正と文字認識工程で使用するAPIについては、無料で使用できる Python の外部ライブラリ（OnnxOCRなど）も検討しましたが、クラウド環境で必要になるサーバーの利用コストを考慮すると外部ライブラリはメモリの消費量が大きく、 VisionAPI の従量課金により生じるコストを上回るため現在の構成を採用しています。
...
- **画像処理 / OCR：** OpenCV / Google Cloud Vision API
- **サーバー：** Gunicorn
- **デプロイ：** Render
